...
var confJson = {day:[], month:[], year:[], dateText:[], titleText:[], href:[]},
	...
	confStyle = {anchor:'text-decoration:none;color:#000;cursor:default;', border:'border:1px solid #6F695A;', family:'verdana,arial;', size:11, bgover:'#d2d2d2', bgout:'#1e4e7e'},
	confHost = {drive:'https://cdn.rawgit.com/MoniAlus/Almanaque/master/'},
...
function createCalendar(json){
	...
	if(confJson.dateText.length){
		...
		contentDate = "\
		<div id='contentDate' style='width:"+widthCalendar+"px'><table style='text-align:center;font-family:"+confStyle.family+"font-size:"+confStyle.size+"px;box-shadow:0 0 10px #333;background-color:#fff'><tr id='chead' style='background-color:"+confStyle.bgout+";'><td><table style='width:218px'><tr><td style='padding:2px;font-family:arial; font-size:"+confStyle.size+"px;text-align:center'><span style='color:#fff'><strong><span id='caption'></span></strong></span></td></tr></table></td></tr><tr><td style='padding:5px;background-color:#fff'><span id='content'></span></td></tr>\
...
function outMonth(obj){
	obj.style.backgroundColor = confStyle.bgout;
}
	...
	for(datePointer=1; datePointer<=numDaysInMonth; datePointer++){
		...
		for(var i=0; i<confJson.dateText.length; i++){	
			if(datePointer == confJson.day[i] && (confOther.monthSelected+1) == confJson.month[i]	&& confOther.yearSelected == confJson.year[i]){
				...
				sStyle += "border-radius:100%;border:2px solid "+confStyle.bgout+";cursor:pointer;";
		...
		for(var i=0; i<links.length; i++){
			if(links[i].className == 'tooltip'){
				links[i].onmouseover = function(){
					 this.setAttribute("tooltip", this.title);
					 this.title = '';
				}
